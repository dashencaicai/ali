"lm.bp" <- function(l1, l2, l1l2 = NULL, l3 = ~1, data, 
                    common.intercept = FALSE, zeroL3 = FALSE, 
  ...
  if(zeroL3) { lambda3 <- rep(0, n) 
    ...
    lambda3 <- rep(max(0.1, cov(unlist(x), unlist(y))), n) } #cov(x,y,use='complete.obs')), n) } 
  #
  #
  ...
  templ1 <- labels(terms(formula(l1)))
  #
  # run this only if there are terms in l1 formula
  if(length(templ1)>0){ 
	  for(k1 in 1:length(templ1)){
		  if(!is.null(l1l2)) { checkvar1 <- sum(labels(terms(formula(l1l2)))==templ1[k1])==1
		  } else{ checkvar1 <- FALSE }		
		checkvar2 <- sum(labels(terms(formula(l2)))==templ1[k1])==1
  ...
  # otherwise use I(internal.data1$indct1*x) to add sepererate parameter on lambda1
  #
  #templ2 <- labels(terms(formula(l2)))
  templ2 <- labels(extractVars(formula(l2)))
  #
  # run this only if there are terms in l1 formula
  if(length(templ2)>0){ 
	  for(k1 in 1:length(templ2)){
		  if(!is.null(l1l2)) {
		    checkvar1 <- (sum(labels(terms(formula(l1l2)))==templ2[k1])+sum(labels(terms(l1))==templ2[k1]))!=2
		  } else{ checkvar1 <- TRUE }		
		  if(checkvar1) { 
  ...
  #
  #
  internal.data1 <- internal.data1 %>% rename(y1y2 = `y1y2.总值`, y1y2_1= `y1y2.总值.1`)
  ...
  #	Calculation of BIC, AIC of Poisson saturated model
	...
	y.mean <- y
	y.mean[y==0] <- 1e-12
	AIC.sat <- sum(log(dpois(unlist(x) , unlist(x.mean))) + 
	               log(dpois(unlist(y) , unlist(y.mean))))
	...
	result <- list(coefficients = allbeta, fitted.values = data.frame(x = m$fitted[1:n], y = m$fitted[(n+1):(2*n)]), 
	  ...
	  while((difllike>pres) && (i <= maxit)) {
	    ...
	    for(j in 1:n) {
	      if(zero[j]) {
	        s[j] <- 0.0; 
	        like[j] <- log(dpois(unlist(x[j]), lambda1[j]))+log(dpois(unlist(y[j]), lambda2[j])) - lambda3[j];
	      }	else {
			    lbp1 <- pbivpois(unlist(x[j])-1, unlist(y[j])-1, lambda=c(lambda1[j], lambda2[j], lambda3[j]), log=TRUE);
			    lbp2 <- pbivpois(unlist(x[j])  , unlist(y[j])  , lambda=c(lambda1[j], lambda2[j], lambda3[j]), log=TRUE);
          #
	    ...
	    #m<-glm( formula1, family=poisson, data=data1 )
	    m <- glm(formula('internal.data1$y1y2[[1]] ~ internal.data1$noncommon'), family = poisson, data = data1)
	    p3 <- length(m$coef)
	    beta <- m$coef
      #	creating names for parameters
	    names(beta) <- newnamesbeta(beta)
